...
def create_dataset_multifield(mode, seed, fmaps, fparams, batch_size, splits, fmaps_norm,
                              rot_flip_in_mem=True, shuffle=True, verbose=False, simulation=None):

    # whether rotations and flippings are kept in memory
    if rot_flip_in_mem:
        data_set = make_dataset_multifield(mode, seed, fmaps, fparams, splits, fmaps_norm, verbose, simulation)
    else:
        data_set = make_dataset_multifield2(mode, seed, fmaps, fparams, splits, fmaps_norm, verbose, simulation)
...
class make_dataset_multifield():
    ...
    def __init__(self, mode, seed, fmaps, fparams, splits, fmaps_norm, verbose, simulation=None):
...
class make_dataset_multifield2():
    ...
    def __init__(self, mode, seed, fmaps, fparams, splits, fmaps_norm, verbose, simulation=None):
        ...
        if simulation == 'TNG':
            idx = np.where(sim_numbers<1000)[0]
            print("TNG:", len(np.where(sim_numbers<1000)[0]))
            sim_numbers = sim_numbers[idx]
            size_sims = len(sim_numbers)
            size_maps = size_sims*splits
            print(size_maps)
        elif simulation == 'SIMBA':
            idx = np.where(sim_numbers>=1000)[0]
            print("SIMBA:", len(np.where(sim_numbers>=1000)[0]))
            sim_numbers = sim_numbers[idx]
            size_sims = len(sim_numbers)
            size_maps = size_sims*splits
            print(size_maps)
